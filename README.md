<div align="center">
  ...
  <h1>
  ONDEWO CSI Client Rust Library
  </h1>
</div>

This library gives a rust application typed, async access to the ONDEWO CSI
(Conversational Speech Interface) gRPC API.

It is generated code plus a thin hand-written surface around it. The interface itself is defined
by the protocol buffer files of the [ONDEWO CSI API](https://github.com/ondewo/ondewo-csi-api),
...
The library is published to [crates.io](https://crates.io/crates/ondewo-csi-client) as
**`ondewo-csi-client`**, with the API documentation on
[docs.rs](https://docs.rs/ondewo-csi-client). Nothing else is needed to consume it - the stubs are
...
* Its version tracks the **ONDEWO CSI API** it was generated from in major and minor - a crate
...
```bash
git clone --recurse-submodules git@github.com:ondewo/ondewo-csi-client-rust.git
cd ondewo-csi-client-rust
...
module path mirrors the proto package: `package ondewo.csi;` is reachable as
`ondewo_csi_client::ondewo::csi`. Run `cargo doc --open` to browse the exact service
and message names of the API version this crate was generated from.

```rust
use ondewo_csi_client::ondewo::csi::*;
use tonic::transport::Channel;
use tonic::Request;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. A channel to the CSI server. `Channel` handles TLS, reconnects and
    //    connection pooling; the generated clients accept any tonic service.
    let channel = Channel::from_static("https://grpc-csi.ondewo.com:443")
        .connect()
        .await?;
    ...
    let token = std::env::var("ONDEWO_CSI_TOKEN")?;
...
```text
.
├── ondewo-csi-api                <----- submodule: the proto definitions to compile
...
1. `checkout_defined_submodule_versions` - checks out the pins at the top of the `Makefile`
   (`ONDEWO_CSI_API_GIT_BRANCH` and `ONDEWO_PROTO_COMPILER_GIT_BRANCH`)
1. `build_compiler` - builds `ondewo-rust-proto-compiler:latest` from the submodule
1. `update_cargo_version` - writes `ONDEWO_CSI_VERSION` into `Cargo.toml`
1. `generate_ondewo_protos` - runs the image over `ondewo-csi-api/ondewo` and writes `src/api`,
...
generated `ConversationsServer` is served over a loopback socket and driven by the generated
`ConversationsClient`, so every declared RPC - the seven unary ones, the bidirectional
`S2sStream` and the server-streaming `GetControlStream` - really is encoded, routed by its
`/ondewo.csi.Conversations/<Method>` path, answered and decoded again. No ONDEWO server is
involved.
...
Releases are cut from the `Makefile`. Bump `ONDEWO_CSI_VERSION`, add the matching

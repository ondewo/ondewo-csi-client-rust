//! ONDEWO CSI (Conversational Speech Interface) gRPC client.
//!
//! Everything under [`api`] is GENERATED from the protos of the `ondewo-csi-api` submodule by the
...
//! `package ondewo.csi;` is reachable as `ondewo::csi::<Message>` and each gRPC service
//! as `ondewo::csi::<service>_client::<Service>Client`.
//!
